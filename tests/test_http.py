...
class HttpTest(unittest.TestCase):
    def test_http_client(self):
        ...
        print(res.read().decode(base.UTF8))  # 自己解码

    def test_urllib_http_post(self):
        # params = json.dumps({"username": "admin", "password": strings.md5("admin123")})  # --- FAIL
        params = json.dumps({"username": "admin", "password": strings.md5("******")}).encode(base.UTF8)  # --- OK
        ...
        msg = res.read().decode(base.UTF8)
        msg_obj = json.loads(msg)
        print(msg_obj)

    def test_urllib_http_get(self):
        params = urllib.parse.urlencode({"sid": "测试用户SID", "other": 111}, encoding=base.UTF8)
