...
def clear_null_char(input_str: str, encoding=base.UTF8):
    return input_str.replace(b'\x00'.decode(encoding=encoding), "")


def clear_null_char_in_file(input_file, encoding=base.UTF8):
...
def md5(msg):
    return hashlib.md5(msg.encode(encoding=base.UTF8)).hexdigest()
